...
Aqui estão os exemplos de objetos para as classes `UsuarioModel` e `PerguntaModel` no formato de código para serem copiados e colados diretamente no seu README.

### Exemplo de Objeto `UsuarioModel`:
...
### Exemplo de Objeto `PerguntaModel`:

```json
{
  "pergunta": "Qual é a principal fonte de energia renovável?",
...
Esses exemplos de objetos JSON podem ser usados como input nas requisições para as rotas de criação de usuários e perguntas, respectivamente.